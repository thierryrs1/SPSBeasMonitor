    ...
    def check_startup_and_restart(beas_services: list[BeasService]):
        ...
        try:
            with db_client.get_connection() as conn:
                ...
                if not row:
                    logger.warning(
                        "[HANA Global] Não foi possível obter o START_TIME em SYS.M_SERVICE_STATISTICS.")
                    return
                ...
                # Regra: se > 5 minutos de vida, e a chave é inédita (nunca
                # processada)
                if diff_minutes >= 5:
                    if last_recorded != start_time_str:
                        logger.warning(
                            f"[HANA Global] HANA rodando há {diff_minutes} minutos (Startup: {start_time_str}). Iniciando restart em MASSA nos serviços BEAS...")

                        for svc in beas_services:
                            SystemMonitor.restart_service(svc.service_name)

                        # Trava o estado para evitar loop nos próximos 1
                        # minutos
                        with open(HanaSystemMonitor.STATE_FILE, "w", encoding="utf-8") as f:
                            f.write(start_time_str)

                        logger.info(
                            f"[HANA Global] Restart em MASSA efetuado com sucesso! Arquivo estado travado em {start_time_str}.")
                    else:
                        logger.debug(
                            f"[HANA Global] Restart massivo do último ligamento já foi executado ({last_recorded}). Ignorando novos comandos.")
                else:
                    logger.info(
                        f"[HANA Global] HANA subiu há apenas {diff_minutes} minutos. Aguardando amadurecer a marca de 5 minutos...")

        except Exception as e:
            logger.error(
                f"[HANA Global] Falha crítica ao auditar a SYS.M_SERVICE_STATISTICS: {e}")
...
class SystemMonitor:
    ...
    def monitor_ram(threshold=85):
        try:
            ...
            logger.info(
                f"[Sistema] [Monitor RAM] Uso atual de RAM: {
                    uso_ram:.2f}%")
            if uso_ram >= threshold:
                logger.warning(
                    f"[Sistema] [Monitor RAM] ⚠️ Alerta: Uso de RAM acima de {threshold}% ({
                        uso_ram:.2f}%)")
        except Exception as e:
            logger.error(
                f"[Sistema] [Monitor RAM] ❌ Erro ao monitorar RAM: {e}")
            ...
            if conn.laddr and conn.laddr.port == port:
                ...
                if pid:
                    try:
                        ...
                        logger.error(
                            f"[Sistema] Erro ao matar PID {pid} na porta {port}: {e}")

        if killed:
            logger.info(
                f"[Sistema] Processos finalizados na porta {port}: {killed}")
        else:
            logger.info(
                f"[Sistema] Nenhum processo encontrado aberto na porta {port}")
        ...
        try:
            ...
            for p in psutil.process_iter(['pid', 'exe', 'cmdline']):
                try:
                    ...
                    # 3. Ambos (cmdline e binpath) são estritamente apenas o
                    # executável
                    ...
                    for arg in p_cmdline:
                        ...
                        if arg_lower == target_svc:
                            is_exact_match = True
                            break
                        # Trata casos como -service=beasService_1 ou
                        # servermodus=beasService10
                        if '=' in arg_lower:
                            if target_svc in arg_lower.split('='):
                                is_exact_match = True
                                break
                        # Trata casos como -service:beasService_1
                        if ':' in arg_lower and len(
                                arg_lower) > 2 and not arg_lower[1] == ':':  # ignorando C:\
                            if target_svc in arg_lower.split(':'):
                                is_exact_match = True
                                break
                    ...
                    if matched:
                        exe_name = p_exe.split(
                            '\\')[-1].lower() if p_exe else "unknown.exe"
                        matching_processes[p.info['pid']] = exe_name
                except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                    pass

            # Verifica se há executáveis duplicados (focado estritamente no
            # beas.exe conforme solicitado)
            ...
            if has_duplicates:
                ...
                logger.warning(
                    f"[{service_name}] [Monitor] ⚠️ Foram detectados processos duplicados para o serviço: {dict(exe_counts)}")
                for pid in pids_to_kill:
                    logger.warning(
                        f"[{service_name}] [Monitor] 🗡️ Forçando finalização no PID {pid} via taskkill...")
                ...
                logger.info(
                    f"[{service_name}] [SC] ▶️ Iniciando serviço novamente: {service_name}")
                subprocess.run(["sc",
                                "start",
                                service_name],
                               stdout=subprocess.DEVNULL,
            ...
            logger.error(
                f"[{service_name}] [Monitor] ❌ Erro ao validar múltiplos PIDs: {e}")
    ...
    def restart_service(service_name: str):
        logger.warning(
            f"[{service_name}] [Monitor] 🔄 Reiniciando serviço do Windows: {service_name}")
        ...
        try:
            ...
            if pid and pid > 0:
                ...
                logger.info(
                    f"[{service_name}] [Monitor] ℹ️ Serviço não possuía PID ativo (já parado).")
            ...
            logger.error(
                f"[{service_name}] [Monitor] ❌ Erro ao obter PID para TASKKILL: {e}")

        logger.info(
            f"[{service_name}] [SC] ▶️ Iniciando serviço: {service_name}")
    ...
    def disable_service(service_name: str):
        logger.warning(
            f"[{service_name}] [SC] 🛑 Desativando o serviço do Windows permanentemente: {service_name}")
        ...
        try:
            ...
            if pid and pid > 0:
                logger.warning(
                    f"[{service_name}] [Monitor] 🗡️ Matando PID {pid} via taskkill antes de desativar...")
                subprocess.run(["taskkill", "/F", "/PID", str(pid)],
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception:
            pass
...
class BeasMonitor:
    ...
    def check_web_portal(self, svc: BeasService):
        ...
        if not alive:
            logger.error(
                f"[{service}] [Portal Web] ❌ {self.local_ip}:{port} fora do ar! Reiniciando em {schema}...")
            SystemMonitor.restart_service(service)
        else:
            logger.info(
                f"[{service}] [Portal Web] ✅ {self.local_ip}:{port} comunicando perfeitamente em {schema}")
    ...
    def check_bsl_odata(self, svc: BeasService):
        ...
        if not svc.html_comm:
            logger.error(
                f"[{service}] [BSL] ❌ html_comm (senha ServiceLayer) não configurado no registry em {schema}.")
            return
        ...
        if not ok:
            logger.warning(
                f"[{service}] [BSL] ❌ OData BSL falhou ({message}). Matando processos e Reiniciando {service} em {schema}...")
            SystemMonitor.kill_port(port)
            SystemMonitor.restart_service(service)
        else:
            logger.info(
                f"[{service}] [BSL] ✅ OData Login BSL bem-sucedido em {schema}.")
    ...
    def check_system_server(self, svc: BeasService):
        ...
        if not schema:
            logger.error(
                f"[{service}] [Atividades] Nenhum schema definido (mssql_database vazio).")
            return
        ...
        try:
            with db_client.get_connection() as conn:
                ...
                # Verifica se o schema realmente existe na base antes de
                # apontar
                cursor.execute(
                ...
                """)
                valida = cursor.fetchone()[0]  # type: ignore

                if valida == 0:
                    logger.warning(
                        f"[{service}] [Atividades] Tabela BEAS_SYS_SERVER não possui registro para 'BEAS - Gerenciamento de Serviço' em {schema}. Inserindo registro padrão...")
                    cursor.execute("""
                        ...
                        SELECT
                            ...
                            CURRENT_TIMESTAMP AS "STATISTIC_LASTSTART",
                    ...
                    conn.commit()
                    logger.info(
                        f"[{service}] [Atividades] Registro inserido com sucesso em {schema}.")

                # Realiza a validação normal de tempo agora que garantimos que
                # o registro existe
                cursor.execute("""
                ...
                # Trata possibilidade remota do select falhar mesmo após o
                # insert
                row = cursor.fetchone()
                if row and row[0] is not None:
                    ...
                    if diff >= 2:
                        logger.warning(
                            f"[{service}] [Atividades] Última execução >= 2 min ({diff} min). Reiniciando {service} em {schema}...")
                        SystemMonitor.restart_service(service)
                    else:
                        if diff < 0:
                            diff = 0
                        logger.info(
                            f"[{service}] [Atividades] Última execução validada há {diff} min em {schema}")
                else:
                    logger.warning(
                        f"[{service}] [Atividades] Registro existe em {schema}, mas STATISTIC_LASTSTART est NULL. Aguardando o Beas rodar as atividades...")
        except Exception as e:
            logger.error(
                f"[{service}] [Atividades] Erro ao validar BEAS_SYS_SERVER: {e} em {schema}")
    ...
    def check_common_heartbeat(self, svc: BeasService, limit):
        ...
        if not schema:
            logger.error(
                f"[{service}] [Beas Common] Nenhum schema definido (mssql_database vazio).")
            return
        ...
        try:
            with db_client.get_connection() as conn:
                ...
                # Check tables
                cursor.execute(
                    f"SELECT COUNT(1) FROM \"SYS\".\"TABLES\" WHERE \"SCHEMA_NAME\" = '{schema}' AND \"TABLE_NAME\" = 'SPS_BEAS_COMMON_HEARTBEAT'")
                cnt = cursor.fetchone()[0]  # type: ignore

                cursor.execute(
                    f"SELECT COUNT(1) FROM \"SYS\".\"PROCEDURES\" WHERE \"SCHEMA_NAME\" = '{schema}' AND \"PROCEDURE_NAME\" = 'SP_SPS_BEAS_COMMON_VALIDATION'")
                cnt2 = cursor.fetchone()[0]  # type: ignore
                ...
                if cnt == 0:
                    cursor.execute(
                        'CREATE TABLE "SPS_BEAS_COMMON_HEARTBEAT" ("ID" INT, "LastUpdate" TIMESTAMP);')
                    cursor.execute(
                        'INSERT INTO "SPS_BEAS_COMMON_HEARTBEAT" ("ID", "LastUpdate") VALUES (1, CURRENT_TIMESTAMP);')
                    conn.commit()
                    logger.warning(
                        f"[{service}] [Beas Common] Tabela SPS_BEAS_COMMON_HEARTBEAT criada com sucesso em {schema}")

                # Valida heartbeat
                cursor.execute(
                    f'SELECT TO_INT(SECONDS_BETWEEN("LastUpdate", CURRENT_TIMESTAMP) / 60) AS DifMinutos FROM {schema}.SPS_BEAS_COMMON_HEARTBEAT WHERE ID = 1;')
                heartbeat = cursor.fetchone()[0]  # type: ignore

                cursor.execute(
                    f'CALL {schema}.SP_SPS_BEAS_COMMON_VALIDATION();')
                cursor.execute(
                    f'DELETE FROM {schema}."BEAS_COMMON_INPUT" WHERE PARAMETER5 = \'SPS Common Check\' AND "Closed" = 1')

                if heartbeat > limit:
                    logger.warning(
                        f"[{service}] [Beas Common] Common travada há ({heartbeat} min). Reiniciando e rodando UPDATE em {schema}...")
                    SystemMonitor.restart_service(service)
                    cursor.execute(f"""
                    ...
                    conn.commit()
                    logger.info(
                        f"[{service}] [Beas Common] Update em BEAS_COMMON_INPUT executado com sucesso em {schema}")
                else:
                    logger.info(
                        f"[{service}] [Beas Common] Common validada há {heartbeat} min em {schema}")

        except Exception as e:
            logger.error(
                f"[{service}] [Beas Common] Erro crítico ao validar HEARTBEAT: {e}")
